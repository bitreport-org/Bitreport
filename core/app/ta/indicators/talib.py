...
def STOCHRSI(
    data, limit, timeperiod=14, fastk_period=14, fastd_period=14, fastd_matype=3
):
    m = 10000000
    fastk, fastd = talib.STOCHRSI(
        m * data["close"], timeperiod, fastk_period, fastd_period, fastd_matype
    )
    return {
...
def SMA(data, limit):
    ...
    for i in range(points2check, 0):
        if dic["fast"][i] < dic["slow"][i] and dic["fast"][i - 1] >= dic["slow"][i - 1]:
            ...
            dic["fast"][i] > dic["slow"][i] and dic["fast"][i - 1] <= dic["slow"][i - 1]
        ):
